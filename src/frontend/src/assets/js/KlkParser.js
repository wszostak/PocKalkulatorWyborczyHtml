...
OpenPKW.KlkParser = {
    ...
    loadPollingStationData: function(xmlDoc, idx) {
        ...
        var xml = xmlDoc;
        var kodGminy = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja/jednostka/' +
                    'jednostka/jednostka/@jnsKod');
        var nrObwodu = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja/jednostka/jednostka' +
            '/jednostka/obwod[' + idx + ']/@obdNumer');
        var siedzibaKomisjiObwodowej = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja' +
            '/jednostka/jednostka/jednostka/obwod[' + idx + ']/obdAdres');
        var gmina = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja/jednostka' +
            '/jednostka/jednostka/jnsNazwa');
        var powiat = '?';
        var wojewodztwo = OpenPKW.KlkParser.getAttribute(xml,
                            '/kalkulator/akcja/jednostka/jednostka/jnsNazwa');
        var numerKomisjiOkregowej = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja' +
            '/jednostka/jednostka/jednostka/organWyborczy/slownik/wpis[@typ=\'NUMER\']');
        var siedzibaKomisjiOkregowej = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja' +
            '/jednostka/jednostka/jednostka/organWyborczy/slownik/wpis[@typ=\'MIEJSCOWNIK_W\']');
        var liczbaWyborcow = OpenPKW.KlkParser.getAttribute(xml, '/kalkulator/akcja/' +
            'jednostka/jednostka/jednostka/obwod[' + idx + ']/obwodPzt/@oodLiczbaWyborcow');

        result = new OpenPKW.PollingStationData(kodGminy, nrObwodu, siedzibaKomisjiObwodowej,
                            gmina, powiat, wojewodztwo, numerKomisjiOkregowej,
                            siedzibaKomisjiOkregowej, liczbaWyborcow);
        return result;
    },
